...
export const exportTimes = async () => {
	...
	const data = solves.map(({_moveCount, _cllCase, _lsepTime, _lseoTime, _cllTime, _block2Time, _unknownTime, mode, date, scramble, time}) => {
		const sequence = new MoveSequence(scramble);
		const scrambleString = sequence.toString();

		return `"${date}";"${formatTime(time)}";"${mode}";"${_unknownTime}";"${_block2Time}";"${_cllTime}";"${_lseoTime}";"${_lsepTime}";"${_cllCase}";"${_moveCount}";"${scrambleString}"`;
	}).join('\n');
